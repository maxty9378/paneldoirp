-- Финальная синхронизация пользователей с правильной обработкой внешних ключей
-- Этот скрипт решает проблему с нарушением внешних ключей
...
-- 5. Временно отключаем проверку внешних ключей
SET session_replication_role = replica;

-- 6. Обновляем внешние ключи в таблице events
UPDATE events 
SET creator_id = um.new_id
FROM user_id_mapping um
WHERE events.creator_id = um.old_id;

-- 7. Обновляем внешние ключи в других таблицах
...
-- 8. Теперь безопасно обновляем ID в таблице users
UPDATE public.users 
SET id = um.new_id
FROM user_id_mapping um
WHERE public.users.id = um.old_id;

-- 9. Включаем обратно проверку внешних ключей
SET session_replication_role = DEFAULT;

-- 10. Создаем недостающих пользователей из auth.users
...
-- 11. Удаляем дубликаты по email (оставляем только с правильным ID)
...
-- 14. Проверяем конкретного пользователя
SELECT 
