/*
  ...
  1. Новые функции
     - `rpc_create_user_v3` - улучшенная версия функции создания пользователей
     - `rpc_sync_user_to_auth` - функция для синхронизации пользователей с auth
     - `rpc_sync_all_users_to_auth` - функция для массовой синхронизации
     - `rpc_test_auth_tables_access` - функция для проверки структуры auth таблиц
  
  2. Исправления
     - Корректно обрабатываем зависимость триггера от функции
     - Используем CASCADE при необходимости
     - Избегаем конфликтов имен функций
*/

-- Сначала удаляем триггер, который зависит от функции
DROP TRIGGER IF EXISTS after_user_created ON users;

-- Удаляем существующие функции с конкретными сигнатурами
DROP FUNCTION IF EXISTS rpc_create_user_v2(text, text, text, text, uuid, uuid, text, text, subdivision_enum, uuid, branch_subrole_enum, text);
DROP FUNCTION IF EXISTS rpc_create_user_v2(text, user_role_enum, text, text, uuid, uuid, text, text, subdivision_enum, uuid, branch_subrole_enum);
DROP FUNCTION IF EXISTS after_user_created_in_users() CASCADE;

-- Создаем новую версию триггерной функции
...
-- Создаем новую функцию создания пользователя
CREATE OR REPLACE FUNCTION rpc_create_user_v3(
  p_full_name text,
...
DROP FUNCTION IF EXISTS rpc_sync_user_to_auth(uuid);
...
DROP FUNCTION IF EXISTS rpc_sync_all_users_to_auth();
...
DROP FUNCTION IF EXISTS rpc_test_auth_tables_access();
...
-- Выдаем права на выполнение новых функций
GRANT EXECUTE ON FUNCTION rpc_create_user_v3 TO authenticated;
