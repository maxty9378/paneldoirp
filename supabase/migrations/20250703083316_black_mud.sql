/*
  # Улучшенное создание пользователей с приоритетом auth.users

  1. Новые функции
    - `rpc_create_user_v2` - создание пользователя сначала в auth.users, затем в public.users
    - `rpc_delete_user_complete` - полное удаление пользователя из обеих систем  
    - `rpc_delete_auth_user` - удаление только auth записи пользователя
  
  2. Логика работы
    - Если указан email, создается auth пользователь с возможностью входа
    - Если email не указан, создается только запись в public.users
