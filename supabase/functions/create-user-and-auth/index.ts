...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: userRole, error: roleError } = await supabaseClient.from('users').select('role').eq('id', user.id).single();
    ...
    const validRoles = ['employee', 'supervisor', 'trainer', 'expert', 'moderator', 'administrator'];
let { email, password = '123456', full_name, role = 'employee', sap_number = null, position_id = null, territory_id = null, phone = null, department = 'management_company', subdivision = 'management_company', branch_id = null, branch_subrole = null, work_experience_days = 0 } = await req.json();
if (!validRoles.includes(role)) {
  role = 'employee';
    ...
    if (email && email.trim()) {
      const { data: existingByEmail } = await supabaseAdmin.from('users').select('*').eq('email', email).single();
      existingUser = existingByEmail;
    }
    if (!existingUser && sap_number && sap_number.trim()) {
      const { data: existingBySap } = await supabaseAdmin.from('users').select('*').eq('sap_number', sap_number).single();
    ...
    if (existingUser) {
          ...
          if (authUser.user) {
            ...
            if (!authError && newAuthData.user) {
              ...
              if (newAuthData.user.id !== userId) {
                ...
                if (newAuthData.user.id !== userId) {
                  const { error: updateIdError } = await supabaseAdmin.from('users').update({
      ...
      const { data: userData, error: userError } = await supabaseAdmin.from('users').update(updateData).eq('id', userId).select().single();
      if (userError) {
        throw userError;
      }
      // Логируем обновление
    ...
    const { data: existingWithId } = await supabaseAdmin.from('users').select('id').eq('id', userId).maybeSingle();
    let userData;
    if (existingWithId) {
      // Валидация роли
      const VALID_ROLES = [
      ...
      console.log('Создаем нового пользователя в базе данных с ID:', userId);
      // Валидация роли
      const VALID_ROLES = [
        'employee',
        'supervisor',
      ...
      const { data, error } = await supabaseAdmin.from('users').insert(insertData).select().single();
      if (error) {
    ...
    return new Response(JSON.stringify({
      success: true,
      message: authCreated ? 'Пользователь успешно создан с возможностью входа' : 'Пользователь создан, но вход невозможен (нет auth записи)',
      user: userData,
      auth_created: authCreated,
      is_update: false,
      tempPassword: authCreated ? password : null
    ...
    try {
      const supabaseAdmin = createClient(Deno.env.get('SUPABASE_URL') ?? '', Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '');
      // Пытаемся получить admin_id из заголовка авторизации
      let adminId = null;
      try {
        const authHeader = req.headers.get('Authorization');
        if (authHeader) {
        ...
        console.error('Не удалось получить admin_id для логирования:', authError);
      }
      await supabaseAdmin.from('admin_logs').insert({
        admin_id: adminId,
