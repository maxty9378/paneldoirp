        ...
        renderContent = (
          ...
          <div className={clsx(
            ...
            {filteredEvents.map((event: EventWithStats, idx: number) => (
              // Чередование цвета карточек: зелёно-teal и голубой
              <div
                key={event.id}
                onClick={() => onNavigateToEvent && onNavigateToEvent(event.id)}
                // Светлый фон, без тени и увеличения, только скругление и border
                className={clsx(
                  "rounded-3xl overflow-hidden cursor-pointer border transition-transform duration-200 hover:-translate-y-1",
                  idx % 2 === 0
                    ? "bg-gradient-to-br from-green-100 to-teal-50 border-green-100"
                    : "bg-gradient-to-br from-sky-100 to-blue-50 border-sky-100"
                )}
              >
                <div className="p-6 md:p-7">
                  {/* Верхний блок: тип, статус, заголовок */}
                  <div className="flex items-start justify-between mb-4">
                    {/* Тип мероприятия — компактный бейдж с иконкой */}
                    <div className="flex-1 min-w-0">
                      ...
                      <h3 className="text-xl font-extrabold text-gray-900 tracking-tight truncate hover:text-teal-700 transition-colors duration-200 mb-1">
                        {event.title}
                      </h3>
                    </div>
                    {/* Бейдж статуса — мягкий фон, цветная иконка */}
                    <div className="flex items-center gap-2">
                      <span className={clsx(
                        "inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold gap-1",
                      ...
                      {canCreateEvents && (
                        <div className="relative">
                          <button className="p-1 text-gray-400 hover:text-teal-600 rounded-full hover:bg-teal-50 transition-colors" title="Дополнительно">
                  ...
                  {/* Описание мероприятия — максимум 2 строки, приглушённо */}
                  {event.description && (
                    <p className="text-gray-500 italic text-sm mb-2 line-clamp-2">
                      {event.description}
                    </p>
                  )}

                  {/* Метаданные мероприятия — в одну строку, аккуратно */}
                  <div className="flex flex-wrap gap-x-6 gap-y-2 items-center mb-4">
                    <div className="flex items-center gap-1 text-sm text-gray-600">
                      <Calendar className="h-4 w-4 text-teal-500" />
                      <span>{event.start_date || event.date_time ? format(new Date(event.start_date || event.date_time as string), 'dd MMMM yyyy', { locale: ru }) : '—'}</span>
                    </div>
                    <div className="flex items-center gap-1 text-sm text-gray-600">
                      <Clock className="h-4 w-4 text-blue-400" />
                      <span>{event.start_date || event.date_time ? format(new Date(event.start_date || event.date_time as string), 'HH:mm') : '—'}</span>
                      {event.end_date && (
                    ...
                    <div className="flex items-center gap-1 text-sm text-gray-600">
                      ...
                      <span className="opacity-60">участников</span>
                      {event.max_participants && (
                        <span className="opacity-60">из {event.max_participants}</span>
                      )}
                    </div>
                  </div>

                  {/* Статистика по тестам — компактно */}
                  {event.participants_count > 0 && (
                    <div className="flex flex-wrap gap-4 text-xs text-blue-800 mb-2">
                      <div>Тест пройден: <b>{event.test_completed_count}</b> из <b>{event.participants_count}</b></div>
                      <div>Не прошли: <b>{event.test_not_passed_count}</b></div>
                      <div>Процент: <b>{event.test_pass_percent}%</b></div>
                    </div>
                  )}
