...
export function EventDetailModal({ isOpen, eventId, onClose }: EventDetailModalProps) {
  ...
  const [testStatus, setTestStatus] = useState({
    entry: { available: false, completed: false, score: null },
    final: { available: false, completed: false, score: null }
  });
  ...
  const fetchEventDetails = async () => {
    ...
    try {
      ...
      // Fetch test status for the current user (if employee)
      if (isEmployee && userProfile?.id) {
        // Check entry test
      ...
      if (!isEmployee) {
        ...
        // Проверяем, есть ли попытки прохождения тестов у текущего пользователя
        if (currentUserId) {
          ...
          setTestStatus(prev => ({
            ...
            entry: {
              ...prev.entry,
              available: true,
              completed: entryAttempts?.status === 'completed',
              score: entryAttempts?.score
            }
          }));
          ...
          setTestStatus(prev => ({
            ...
            final: {
              ...prev.final,
              available: true,
              completed: finalAttempts?.status === 'completed',
              score: finalAttempts?.score
            }
          }));
        }
      }
  ...
  const startTest = (testType: 'entry' | 'final') => {
    // В реальном приложении здесь будет логика запуска теста
    alert(`Переход к ${testType === 'entry' ? 'входному' : 'финальному'} тесту`);
  };
