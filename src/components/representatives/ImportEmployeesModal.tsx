...
export function ImportEmployeesModal({ isOpen, onClose, onSuccess }: ImportEmployeesModalProps) {
  ...
  const handleImport = async () => {
    ...
    setImporting(true);
    try {
      // Здесь будет логика импорта сотрудников
      await new Promise(resolve => setTimeout(resolve, 2000));
      setResult({ success: 5, errors: [] });
  ...
  const downloadTemplate = () => {
    ...
    const headers = ['ФИО', 'Email', 'SAP номер', 'Телефон', 'Должность', 'Территория'];
    const template = [
      headers.join(','),
      'Иванов Иван Иванович,ivanov@example.com,12345,+7900123456,Менеджер,Москва'
    ].join('\n');
            ...
            <>
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500 mt-2">
                  Скачайте шаблон и заполните данными сотрудников
                </p>
              </div>
            ...
            <div className="space-y-4">
              <div className={`p-4 rounded-lg ${result.success > 0 ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
                ...
                <p className={`text-sm mt-1 ${result.success > 0 ? 'text-green-700' : 'text-red-700'}`}>
                  Успешно импортировано: {result.success} сотрудников
                </p>
