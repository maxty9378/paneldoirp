...
# Использование: sudo bash setup-https-letsencrypt.sh
...
echo -e "${GREEN}3️⃣ Получение SSL-сертификата...${NC}"
echo -e "${BLUE}⚠️  Важно: Домен $DOMAIN должен указывать на этот сервер!${NC}"
echo ""
...
echo -e "${GREEN}4️⃣ Настройка автообновления сертификата...${NC}"

# Добавляем cron задачу для автообновления
(crontab -l 2>/dev/null | grep -v certbot; echo "0 3 * * * certbot renew --quiet --post-hook 'systemctl reload nginx'") | crontab -

# Шаг 5: Проверка
echo -e "${GREEN}5️⃣ Проверка HTTPS...${NC}"
