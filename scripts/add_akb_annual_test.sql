-- Добавление годового теста "Управление территорией для развития АКБ"
-- Тест аналогичен входному, но без варианта "Затрудняюсь ответить"

DO $$
DECLARE
  akb_annual_test_id UUID;
  question_id UUID;
...
BEGIN
  ...
  VALUES (
    'Годовой тест "Управление территорией для развития АКБ"',
    'Проверка знаний по управлению территорией и развитию АКБ спустя 3 месяца после обучения',
    'annual',
    75, -- проходной балл 75% (выше чем у входного)
    45, -- 45 минут на выполнение
    in_person_type_id,
    'active'
  )
  RETURNING id INTO akb_annual_test_id;

  RAISE NOTICE 'Создан годовой тест с ID: %', akb_annual_test_id;
  ...
  VALUES (akb_annual_test_id, 'Какая самая частая причина возникновения возражений при продаже?', 'single_choice', 1, 1)
  ...
  INSERT INTO test_questions (test_id, question, question_type, "order", points)
  VALUES (akb_annual_test_id, 'Какие преимущества дает ТП соблюдение технологии продаж «8 этапов визита в ТТ»?', 'single_choice', 2, 1)
  RETURNING id INTO question_id;
  ...
  INSERT INTO test_questions (test_id, question, question_type, "order", points)
  VALUES (akb_annual_test_id, 'Рассчитайте прибыль в рублях, которую получит клиент от продажи одной банки при закупке партии E-ON по акции «10+6». При условии: 1) входная цена без акции составляла 65 р.; 2) на полке E-ON будет продаваться по 85 р.', 'single_choice', 3, 1)
  RETURNING id INTO question_id;
  ...
  INSERT INTO test_questions (test_id, question, question_type, "order", points)
  VALUES (akb_annual_test_id, 'Какие проблемы начинаются у ТП, когда он перестает анализировать итоги визита в ТТ, используя маршрутный лист?', 'single_choice', 4, 1)
  RETURNING id INTO question_id;
  ...
  INSERT INTO test_questions (test_id, question, question_type, "order", points)
  VALUES (akb_annual_test_id, 'Какими правилами составления маршрута нужно пользоваться с целью сокращения времени работы?', 'single_choice', 5, 1)
  RETURNING id INTO question_id;
  ...
  INSERT INTO test_questions (test_id, question, question_type, "order", points)
  VALUES (akb_annual_test_id, 'Какая тактика планирования в начале месяца позволит выполнить план ТП на 100% даже при наличии каких-либо форс-мажоров?', 'single_choice', 6, 1)
  RETURNING id INTO question_id;
  ...
  INSERT INTO test_questions (test_id, question, question_type, "order", points)
  VALUES (akb_annual_test_id, 'ЛПР высоко проходимой ТТ говорит, что у него рядом работает сетевой магазин, где Fresh Bar дешевле. Какие условия ТП может предложить ЛПР для успешной конкуренции с сетевым магазином?', 'single_choice', 7, 1)
  RETURNING id INTO question_id;
  ...
  INSERT INTO test_questions (test_id, question, question_type, "order", points)
  VALUES (akb_annual_test_id, 'Какая тактика наиболее эффективна в работе с высоко объемными ТТ, которые находятся в проходимом месте, берут все товары СНС и имеют покупателей с хорошим доходом?', 'single_choice', 8, 1)
  RETURNING id INTO question_id;
  ...
  INSERT INTO test_questions (test_id, question, question_type, "order", points)
  VALUES (akb_annual_test_id, 'Какая тактика наиболее эффективна в работе с низко объемными ТТ, которые имеют низкую проходимость, делают минимальные заказы и обслуживают покупателей с небольшим доходом?', 'single_choice', 9, 1)
  RETURNING id INTO question_id;
  ...
  INSERT INTO test_questions (test_id, question, question_type, "order", points)
  VALUES (akb_annual_test_id, 'Какой этап визита нужно усилить в работе ТП, когда он часто слышит возражение: «Ваш товар не продается»?', 'single_choice', 10, 1)
  RETURNING id INTO question_id;
  ...
  RAISE NOTICE 'Годовой тест "Управление территорией для развития АКБ" успешно создан с 10 вопросами (без варианта "Затрудняюсь ответить")';
