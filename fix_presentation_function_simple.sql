-- Простая версия функции assign_presentation_numbers_batch
-- Просто перезаписываем назначения без очистки и проверки конфликтов

-- Удаляем старую версию функции
DROP FUNCTION IF EXISTS assign_presentation_numbers_batch(UUID, TEXT, UUID);

-- Создаем простую версию с перезаписью
...
BEGIN
    ...
    BEGIN
        ...
        -- чтобы избежать конфликтов с уникальным индексом на (exam_event_id, presentation_number)
        ...
        LOOP
            ...
            -- Вставляем новое назначение (конфликтов быть не должно, так как мы очистили старые)
...
COMMENT ON FUNCTION assign_presentation_numbers_batch(UUID, TEXT, UUID) IS 'Назначение номеров выступлений с очисткой старых данных для избежания конфликтов';
