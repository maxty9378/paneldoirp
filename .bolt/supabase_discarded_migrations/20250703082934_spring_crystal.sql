/*
  # Улучшенное создание пользователей

  1. Новые функции
     - `rpc_create_user_v2` - создает пользователя сначала в auth.users, затем в public.users
     - `rpc_delete_user_complete` - полное удаление пользователя из обеих систем
     - `rpc_delete_auth_user` - удаление только из auth.users

  2. Изменения
     - Исправлены ошибки при работе с enum типами для ролей
     - Улучшено ведение логов операций
     - Добавлена защита от создания дубликатов учетных записей

  3. Безопасность
     - Проверки на наличие прав администратора или модератора
     - Безопасная обработка ошибок с записью логов
*/
